...
export class UserDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.form = fb.group({
      isAdministrator: [data.user?.roles?.includes('Administrator'), Validators.required],
      isFridge: [data.user?.roles?.includes('Fridge'), Validators.required],
    });
  }
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      this.submit.pipe(
        ...
        switchMap(() => of({error: 'not implemented'}
        ).pipe(
          catchError(e => {
